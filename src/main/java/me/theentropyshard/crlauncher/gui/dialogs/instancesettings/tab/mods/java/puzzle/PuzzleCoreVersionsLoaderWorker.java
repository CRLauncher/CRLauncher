...
package me.theentropyshard.crlauncher.gui.dialogs.instancesettings.tab.mods.java.puzzle;
...
public class PuzzleCoreVersionsLoaderWorker extends Worker<List<GithubRelease>, Void> {
    ...
    public PuzzleCoreVersionsLoaderWorker(JComboBox<GithubRelease> versionsCombo, ToggleableItemListener listener, CosmicInstance instance) {
        super("loading Puzzle Core versions");
    ...
    protected List<GithubRelease> work() throws Exception {
        List<GithubRelease> versionArray = new GithubApi().getAllReleases("PuzzlesHQ", "puzzle-loader-core");

        if (this.instance.getPuzzleCoreVersion() == null) {
            this.instance.setPuzzleCoreVersion(versionArray.get(0).tag_name);
        }
    ...
    protected void done() {
        ...
        for (int i = 0; i < this.versionsCombo.getItemCount(); i++) {
            ...
            if (release.tag_name.equals(this.instance.getPuzzleCoreVersion())) {
