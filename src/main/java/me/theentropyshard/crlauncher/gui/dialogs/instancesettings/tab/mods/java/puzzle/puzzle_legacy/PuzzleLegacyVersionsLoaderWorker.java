...
package me.theentropyshard.crlauncher.gui.dialogs.instancesettings.tab.mods.java.puzzle.puzzle_legacy;
...
public class PuzzleLegacyVersionsLoaderWorker extends Worker<List<GithubRelease>, Void> {
    ...
    public PuzzleLegacyVersionsLoaderWorker(JComboBox<GithubRelease> versionsCombo, ToggleableItemListener listener, CosmicInstance instance) {
        super("loading Puzzle Legacy versions");
    ...
    protected List<GithubRelease> work() throws Exception {
        List<GithubRelease> versionArray = new GithubApi().getAllReleases("PuzzlesHQ", "PuzzleLoader");

        if (this.instance.getPuzzleLegacyVersion() == null) {
            this.instance.setPuzzleLegacyVersion(versionArray.get(0).tag_name);
        }
    ...
    protected void done() {
        ...
        for (int i = 0; i < this.versionsCombo.getItemCount(); i++) {
            ...
            if (release.tag_name.equals(this.instance.getPuzzleLegacyVersion())) {
